...
namespace Units {
	...
	for (auto j = 0; j < x; ++j) {
		for (auto i = 0; i < y; ++i) {
			...
			Scenes::scene.emplace<Components::Position_X>(archer, 0.0f, 150.0f + (i * 144.0f), 0.0f);
			Scenes::scene.emplace<Components::Position_Y>(archer, 0.0f, 200.0f + (j * 144.0f), 0.0f);
